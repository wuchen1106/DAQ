    ...
    TGenericData(bklen, bktype, name, pdata)
{
  ...
  for(int i = 0; i < GetSize(); i++){
    ...
    if( (word & 0xf8000000) == 0x00000000){
      fMeasurements.push_back(TDCMeasurement(current_tdc_header,word));
    }
    ...
    if( (word & 0xf8000000) == 0x88000000) fExtendedTriggerTimeTag = (word & 0x07ffffff);

    // Found global trailer
    if( (word & 0xf8000000) == 0x80000000){ // Found trailer
      found_trailer_word = i;
      fWordCount = (word & 0x0001fffe0) >> 5;
      if(fWordCount != GetSize())
	std::cerr << "Error in decoding V1190 data; word count in trailer ("<<fWordCount 
		  << ") doesn't match bank size ("<< GetSize() << ")." << std::endl;
      fStatus = (word & 0x07000000) >> 24;
    }
