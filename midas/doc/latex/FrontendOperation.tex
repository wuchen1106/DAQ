...
\label{FrontendOperation_idx_hardware}
...
From a software point of view, we keep this same term, i.e. \char`\"{}Equipment\char`\"{}, to refer to the means of collecting the data related to this set of hardware equipment. \par
...
\label{FrontendOperation_idx_data_transfer_optimize}
...
\label{FrontendOperation_idx_frontend_event_triggers}
...
\label{FrontendOperation_idx_event_readout}
...
An example of an Equipment called \char`\"{}Scaler\char`\"{} is shown below: 
\begin{DoxyCode}
...
\end{DoxyCode}
\hypertarget{FE_ODB_equipment_tree_FE_ODB_equipment_common}{}\subsubsection{The ODB /Equipment/$<$equipment-\/name$>$/Common subtree}\label{FE_ODB_equipment_tree_FE_ODB_equipment_common}
...
\hline
Keys in the ODB /Equipment/Common subtree   \\\cline{1-1}
Key  &Type\par
  &Explanation 
...
\label{FE_ODB_equipment_tree_idx_hotlink_Equipment-common-parameters}
...
Equipment Settings hot-\/linked in ODB\par
   \\\cline{1-1}
$\ast$$\ast$ \hyperlink{FE_table_FE_tbl_EqEnabled}{Enabled} flag   \\\cline{1-1}
$\ast$$\ast$ \hyperlink{FE_table_FE_tbl_ReadOn}{Read-\/On (RO\_\-XX)} flags   \\\cline{1-1}
$\ast$$\ast$ \hyperlink{FE_table_FE_tbl_Period}{Period}   \\\cline{1-1}
...
\begin{DoxyPre}[/Equipment/Trigger/Common]\end{DoxyPre}
...
The \hyperlink{FE_table_FE_tbl_Format}{formats} for the ADC0 bank is \char`\"{}fixed\char`\"{}, and for the TDC0 bank \char`\"{}midas\char`\"{}.\par
...
\label{FE_table_idx_Equipment_List_Parameters}
\hypertarget{FE_table_idx_Equipment_List_Parameters}{}
 \label{FE_table_idx_Equipment_Types}
\hypertarget{FE_table_idx_Equipment_Types}{}
 \begin{table}[h]\begin{TabularC}{3}
\hline
\multirow{1}{\linewidth}{EQUIPMENT Parameter\par
  }&\multirow{1}{\linewidth}{Explanation\par
   }\\\cline{1-2}
\multirow{1}{\linewidth}{ \label{FE_table_FE_tbl_EqName}
\hypertarget{FE_table_FE_tbl_EqName}{}
 \label{FE_table_idx_Equipment_name}
\hypertarget{FE_table_idx_Equipment_name}{}
  Equipment Name\par
  }&\multirow{1}{\linewidth}{Each equipment has to be assigned a {\bfseries  unique name } (i.e. no two equipments in the same experiment may have the same name). The name will be the reference name of the equipment generating the event. Examples of typical Equipment Names are \char`\"{}Trigger\char`\"{},\char`\"{}Scaler\char`\"{},\char`\"{}Epics\char`\"{}.   }\\\cline{1-2}
\multirow{17}{\linewidth}{1\par
  }&\multirow{1}{\linewidth}{\hyperlink{structEQUIPMENT__INFO}{EQUIPMENT\_\-INFO} Structure\par
  

}&\multirow{17}{\linewidth}{1\par


}\\\cline{1-15}\cline{1-14}
...
\hypertarget{FE_table_idx_equipment_flags}{}
 ...
 \begin{table}[h]\begin{TabularC}{3}
\hline
\multirow{1}{\linewidth}{Equipment Types\par
  }&Explanation \par
   \\\cline{1-2}
...
\hypertarget{FE_table_idx_equipment_flags_slow-control}{}
 EQ\_\-SLOW &Slow Control Event  &This flag declares the equipment as a Slow Control Equipment. This will enable the call to the {\bfseries idle} function part of the class driver. This flag also causes event data to be sent automatically to the /equipment/variables tree in the ODB.  \\\cline{1-3}
...
\hypertarget{FE_table_idx_equipment_flags_event-builder}{}
 EQ\_\-EB &Event run through the event builder  &This flag identifies the equipment as a {\bfseries fragment event} and should be ORed with the EQ\_\-POLLED in order to be identified by the \hyperlink{FE_Event_Builder}{Event Builder Functions}. Not to be confused with a fragmented event (see above)!   \\\cline{1-3}
\end{TabularC}
\centering
...
The \hyperlink{FE_eq_event_routines_FE_poll_or_interrupt_readout_routine}{example} shows a CAMAC or VME source for the interrupt. A MACRO (LAM\_\-SOURCE) calculates the bit pattern from the \hyperlink{structparameters}{parameters} (Crate,Slot). If not using an interrupt, this field can be set to 1.   \\\cline{1-2}
...
\hypertarget{FE_table_idx_equipment_enable}{}
  Enabled \hyperlink{FE_eqdec_FE_equipment_odb_common}{$\ast$$\ast$}   &\char`\"{}Enable\char`\"{} switch for the equipment. Only when enabled (i.e. TRUE) is this equipment active.   \\\cline{1-2}
...
\hypertarget{FE_table_idx_equipment_periodic_time-interval}{}
 ...
 Period \hyperlink{FE_eqdec_FE_equipment_odb_common}{$\ast$$\ast$}\par
  &Time interval for \hyperlink{FE_table_FE_tbl_EqPeriodic}{Periodic} equipment or time out value in case of \hyperlink{FE_table_FE_tbl_EqPolled}{Polled} or \hyperlink{FE_table_FE_tbl_EqMultithread}{Multithread} equipment (units are milliseconds).   \\\cline{1-2}
...
\hypertarget{FE_table_FE_tbl_EventLimit}{}
 Event Limit \hyperlink{FE_eqdec_FE_equipment_odb_common}{$\ast$$\ast$}\par
  &Specifies the number of events to be taken prior to forcing an End-\/of-\/Run transition. The value 0 disables this option.   \\\cline{1-2}
...
\hypertarget{FE_table_FE_tbl_Reserved}{}
 Reserved\par
  &Reserved field for system (used for \hyperlink{FE_ODB_equipment_tree_FE_frontend_host}{Frontend hostname}). Should be present and set to \char`\"{}\char`\"{} in the frontend.   \\\cline{1-2}
Reserved  &Reserved field for system (used for \hyperlink{FE_ODB_equipment_tree_FE_frontend_name}{Frontend name}). Should be present and set to \char`\"{}\char`\"{} in the frontend.  \\\cline{1-2}
Reserved  &Reserved field for system (used for \hyperlink{FE_ODB_equipment_tree_FE_frontend_file_name}{Frontend file name}). Should be present and set to \char`\"{}\char`\"{} in the frontend.   \\\cline{1-2}
\multirow{1}{\linewidth}{\par
 End of \hyperlink{structEQUIPMENT__INFO}{EQUIPMENT\_\-INFO} structure \par
   }\\\cline{1-1}
\multirow{1}{\linewidth}{\label{FE_table_FE_tbl_ReadoutRoutine}
\hypertarget{FE_table_FE_tbl_ReadoutRoutine}{}
 Readout routine\par
  }&\multirow{1}{\linewidth}{This field contains the name of the function to be called when the event is triggered. The name of this routine is determined by the user. This readout routine must be present in the frontend code. It will be called at an \label{FE_table_FE_tab_event_trigger}
\hypertarget{FE_table_FE_tab_event_trigger}{}
 {\bfseries event trigger}, which will occur under one of these conditions depending on the \hyperlink{FE_table_FE_tbl_EqType}{Equipment type}. 
\begin{DoxyItemize}
\item {\bfseries polled mode}: the \hyperlink{mfe_8c_a98cc3257284af91fa0e8da10ddf227c8}{poll\_\-event()} routine has detected a trigger request while polling on a trigger source. 
\item {\bfseries interrupt mode}: an interrupt has occurred whose source is pre-\/defined in the \hyperlink{mfe_8c_a681a5d1fe2e8a4735bcf03824ffb3e81}{interrupt\_\-configure()} routine. 
\item {\bfseries periodic modes}: trigger occurs periodically at a time interval specified by the \hyperlink{FE_table_FE_tbl_Period}{Period} field  
\item \label{FE_table_FE_Manual_trig_mode}
\hypertarget{FE_table_FE_Manual_trig_mode}{}
 {\bfseries manual trigger mode}: trigger occurs when 
\begin{DoxyItemize}
\item the \hyperlink{FE_eq_event_routines_FE_manual_trigger}{Manual Trigger} button is pressed on the web interface 
\item a client requests an event by \hyperlink{FE_eq_event_routines_FE_backend_Manual_Trigger}{triggering the event sending mechanism} via a RPC. 
\end{DoxyItemize}
\end{DoxyItemize}Click for further details of a \hyperlink{FE_eq_event_routines_FE_poll_event}{poll\_\-event routine} and an \hyperlink{FE_eq_event_routines_FE_interrupt_event}{interrupt routine}.   }\\\cline{1-2}
\multirow{1}{\linewidth}{\label{FE_table_FE_tbl_ClassDriver}
\hypertarget{FE_table_FE_tbl_ClassDriver}{}
 Class Driver }&\multirow{1}{\linewidth}{Reserved for Class Driver main routine. If \hyperlink{FE_table_FE_tbl_EqType}{Equipment Type} is {\bfseries  not EQ\_\-SLOW }, this field must be present but set to NULL.\par
   }\\\cline{1-2}
\multirow{1}{\linewidth}{\label{FE_table_FE_tbl_DevDriverList}
\hypertarget{FE_table_FE_tbl_DevDriverList}{}
 Device Driver List }&\multirow{1}{\linewidth}{Reserved for Device Driver List. If \hyperlink{FE_table_FE_tbl_EqType}{Equipment Type} is {\bfseries  not EQ\_\-SLOW }, this field must be present, but set to NULL\par
   }\\\cline{1-2}
\multirow{1}{\linewidth}{\label{FE_table_FE_tbl_BankDef}
\hypertarget{FE_table_FE_tbl_BankDef}{}
 Bank Definition or \label{FE_table_FE_tbl_InitString}
\hypertarget{FE_table_FE_tbl_InitString}{}
 Init String \par


}&\multirow{1}{\linewidth}{This field contains either 
\begin{DoxyItemize}
\item Bank declaration if saving data in ROOT format (see \hyperlink{FE_table_FE_tbl_Format}{Format}) or 
\item Init String (\label{FE_table_FE_tbl_Format}
\hypertarget{FE_table_FE_tbl_Format}{}
 \char`\"{}FIXED event format\char`\"{} only) or 
\item NULL 
\end{DoxyItemize}Since the introduction of {\bfseries ROOT} , the frontend is required to contain the definition of the banks when the raw data is to be converted to and stored in {\bfseries ROOT} format. The conversion is needed since there is no ROOT \hyperlink{FE_table_FE_tbl_Format}{format} supported. This procedure is equivalent to the bank declaration in the analyser. See \hyperlink{FE_eqdec_FE_ROOT_Structure_Def}{ROOT structure definition for frontend EQUIPMENT list} example for details. \par
 \par
...
\end{TabularC}
\centering
\caption{TABLE showing Equipment List Parameters }
\end{table}
\par
...
\hline
Hardware :  &Filename :  &Directory :   &Language :  \\\cline{1-4}
VME &\hyperlink{fevmemodules_8c}{fevmemodules.c} &examples/experiment &C  \\\cline{1-4}
CAMAC &\hyperlink{frontend_8c}{frontend.c} &../examples/experiment  &C  \\\cline{1-4}
VME &\hyperlink{fevme_8cxx}{fevme.cxx} &../examples/Triumf/c++   &C++ \\\cline{1-4}
\end{TabularC}
\centering
...
\label{Frontend_code_idx_CAMAC_LAM-macro}
...
\end{DoxyItemize}\hypertarget{FE_Super_Event_FE_Super_Event_readout}{}\paragraph{Example Readout code for Super-\/Event}\label{FE_Super_Event_FE_Super_Event_readout}
...
EB                              DIR
    ...
    Settings                    DIR
        ...
        Fragment Required       BOOL    2     4     9s   0   RWD
                                        ...
                                        [1]             y
\end{DoxyCode}
\hypertarget{FE_Event_Builder_FE_eb_Operation}{}\paragraph{Event Builder Operation}\label{FE_Event_Builder_FE_eb_Operation}
...
\end{DoxyItemize}
 ...
 \hypertarget{FE_utils_FE_dio_utility}{}\subparagraph{dio          -\/ direct I/O driver}\label{FE_utils_FE_dio_utility}
